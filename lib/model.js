  ...
  Object.keys(attributes).forEach(function (attr) {
    ...
    modelAttributes[attr] = {
      ...
      set: function setter(value) {
        var prevValue = this._data[attributeName];
        ...
        this._data[attributeName] = types.check(attribute.type, value, this._data[attributeName], attr);
...
function modelToJson() {
  ...
  if (attributes) {
    ...
    for (i = 0, iLen = keys.length; i < iLen; ++i) {
      key = keys[i];
      jsonKey = attributes[key].alias || key;
