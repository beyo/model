/**
FIXME : this module should not be a repository the models, but a type validation module!
        the `register` function should register a type and should generate a type validation function!
        Consequently, types should be defined as an object of key->value where the keys are the
        full qualitied model name (on a single level, no recursive objects).
        It should expose :
          - exceptions : an object of type exceptions
          - register : a function to register types for validation
          - unregister : a function to unregister types for validation
          - check : a function to check if a given value is of a given type (and return the value)
            registeredNames : an array of registered type names
*/
