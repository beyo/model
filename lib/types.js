...
function checkType(type, value, previous, attributeName) {
  ...
  if (typeof type !== 'string') {
    throw TypeException('Type name must be a string `{{type}}`', null, [ attributeName ], { type: String(type) });
  }

  validator = primitives[type.toLocaleLowerCase()] || registry[type];

  if (!validator) {
    throw TypeException('Unknown type `{{type}}`', null, [ attributeName ], { type: type });
  }
