...
function parseType(type) {
  ...
  if (type instanceof Function) {
    ...
    throw new TypeError('Type name must be a string : ' + stringify(type));
  }
...
function defineType(name, type, validator) {
  ...
  if (typeof name !== 'string') {
    throw new TypeError('Invalid type name : ' + stringify(name));
  } else if (typeof type !== 'function') {
    throw new TypeError('Invalid type : ' + stringify(type));
...
function undefineType(type) {
  ...
  if (typeDef.isArray) {
    throw new TypeError('Cannot undefine array type : ' + stringify(type));
  } else if (primitives[typeDef.name]) {
    throw new TypeError('Cannot undefine primitive type : ' + stringify(type));
  }
...
/**
Check if the given type is defined
